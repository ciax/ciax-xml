  ...
  module Frm
    ...
    class Field
      ...
      module Conv
        ...
        def conv(ent)
          ...
          ___make_data(ent.deep_subst_par(@seldb.get(ent[:response])))
          time_upd
          verbose { _conv_text('Frame -> Field', ent.id, time_id) }
          self
        end
    ...
    if $PROGRAM_NAME == __FILE__
      ...
      Opt::Conf.new('[id] [cmd]', options: 'h') do |cfg|
        ...
        field.sub_stat.cmode(cfg.opt.host)
        atrb = field.dbi.pick(:stream).update(stat: field)
