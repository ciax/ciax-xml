...
module CIAX
  ...
  class ExtCmd < Command
    def initialize(db)
      super()
      sv=self['sv']
      sv['ext']=ExtGrp.new(db,[sv.procs]){|id,pa|
  ...
  class ExtGrp < Group
    def initialize(db,procary)
      type?(db,Db)
      super({'color' => '6','caption' => "External Commands"},procary)
      @cmdary=[]
      cdb=db[:command]
